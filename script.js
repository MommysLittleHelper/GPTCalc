...
function updateCalculationStatus(validCount, warningCount) {
    var box = document.getElementById('calculationStatus');
    ...
    if (validCount === 0) {
        ...
        icon.textContent = '🔴';
        title.textContent = 'Расчёт не выполнен';
        text.textContent = 'Надёжные группы габаритов не найдены';
    } else if (warningCount > 0) {
        box.classList.add('warning');
        icon.textContent = '🟡';
        title.textContent = 'Расчёт требует проверки';
        text.textContent = warningCount === 1
            ? '1 позиция имеет автоматически определённую единицу измерения'
            : warningCount + ' позиций имеют автоматически определённую единицу измерения';
    } else {
        icon.textContent = '🟢';
        title.textContent = 'Расчёт завершён';
        text.textContent = 'Распознано без предупреждений: ' + validCount + (validCount === 1 ? ' позиция' : ' позиций');
    }
}

function renderResults() {
    var totalVolume = 0, totalPieces = 0, detailsHtml = '';
    var validCount = 0, warningCount = 0;
    ...
    for (var i = 0; i < parsedItems.length; i++) {
        ...
        validCount++;
        if (item.isDoubtful) warningCount++;
    ...
    updateCalculationStatus(validCount, warningCount);
