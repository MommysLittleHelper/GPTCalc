...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Автоматический расчёт после вставки.
    // Используем input + небольшую задержку: это надёжнее, чем
    // рассчитывать только событие paste (работает и для вставки
    // через контекстное меню, drag&drop и автозаполнение).
    ...
    function scheduleAutoCalculate() {
        ...
        var text = mainTextarea.value.trim();
        if (!text || text === lastAutoText) return;

        autoCalcTimer = setTimeout(function() {
            if (!mainTextarea.value.trim()) return;
            lastAutoText = mainTextarea.value.trim();
            calculate();
        }, 300);
    }

    if (mainTextarea) {
        mainTextarea.addEventListener('input', scheduleAutoCalculate);

        // Отдельно оставляем paste как страховку для браузеров/расширений,
        // которые не всегда корректно генерируют input после вставки.
        mainTextarea.addEventListener('paste', function() {
            setTimeout(scheduleAutoCalculate, 50);
        });
    }

    // Кнопка адаптации: не заменяет расчёт и не меняет исходный текст.
    // Она принудительно запускает анализ текущего текста и показывает
    // результат в обычном блоке расчёта.
    var adaptButton = document.getElementById('adaptBtn');
    if (adaptButton) {
        adaptButton.addEventListener('click', function() {
            calculate();
        });
    }
});