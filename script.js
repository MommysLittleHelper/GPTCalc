...
function parseLine(line, globalStart) {
    ...
    while ((m = sepRe.exec(work)) !== null) {
        ...
        var tail = work.slice(end, Math.min(work.length, end + 30));
        var qMatch = tail.match(/^\\s*(?:[xх×*]\\s*)?(\\d+(?:[.,]\\d+)?)\\s*(?:шт\\.?|штук|мест(?:а|о)?\\b)?/i);
        var quantity = 1, qLen = 0;
        if (qMatch) {
            var q = normalizeNumber(qMatch[1]);
            if (q >= 1 && q <= 10000 && !/^(?:19|20)\\d{2}$/.test(qMatch[1])) {
                quantity = q; qLen = qMatch[0].length;
            }
        }
    ...
    while ((m = wsRe.exec(work)) !== null) {
        ...
        var endTriple = end2;
        var tail2 = work.slice(endTriple, Math.min(work.length, endTriple + 30));
        var q2 = tail2.match(/^\\s*(\\d+(?:[.,]\\d+)?)\\s*(шт\\.?|штук|мест(?:а|о)?\\b)/i);
        var quantity2 = q2 ? normalizeNumber(q2[1]) : 1;
        if (!(quantity2 >= 1 && quantity2 <= 10000)) quantity2 = 1;
