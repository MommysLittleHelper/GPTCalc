...
function quantityFromContext(t,start,end){
 ...
 const after=t.slice(end,Math.min(t.length,end+220));

 // Strong signals first: explicit quantity labels anywhere after the dimensions.
 let m=after.match(/(?:^|[,\s;:()\-—–])(?:кол-?\s*во|количество|qty|quantity)\s*[:=\-]?\s*(\d+(?:[.,]\d+)?)\s*(?:шт\.?|штук|мест(?:а|о)?|pcs?)?/i);
 if(m)return Math.max(1,Math.round(num(m[1])));

 // "8 шт.", "8 штук", "8 мест" after the dimensions.
 m=after.match(/(?:^|[,\s;:()\-—–])(\d+(?:[.,]\d+)?)\s*(шт\.?|штук|мест(?:а|о)?|pcs?)\b/i);
 if(m)return Math.max(1,Math.round(num(m[1])));

 // Quantity immediately after the unit: "4.5 метров, 8шт".
 m=after.match(/^\s*(?:мм|см|м|mm|cm|m|миллиметр(?:а|ов)?|сантиметр(?:а|ов)?|метр(?:а|ов)?)\s*(?:,|;|:|\-|\u2014|\u2013)?\s*(\d+(?:[.,]\d+)?)\s*(?:шт\.?|штук|мест(?:а|о)?|pcs?)\b/i);
 if(m)return Math.max(1,Math.round(num(m[1])));

 // "× 4" / "x 4" after the dimensions.
 m=after.match(/^\s*(?:мм|см|м|mm|cm|m|миллиметр(?:а|ов)?|сантиметр(?:а|ов)?|метр(?:а|ов)?)?\s*[×x*]\s*(\d+(?:[.,]\d+)?)/i);
 if(m)return Math.max(1,Math.round(num(m[1])));

 // Explicit quantity before dimensions: "2 шт. 600x400x300".
 m=before.match(/(?:^|[\s,;:])(?:кол-?\s*во|количество|qty|quantity)\s*[:=\-]?\s*(\d+(?:[.,]\d+)?)\s*(?:шт\.?|штук|мест(?:а|о)?|pcs?)?/i);
 if(m)return Math.max(1,Math.round(num(m[1])));
