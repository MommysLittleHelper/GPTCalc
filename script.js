...
// --- v1.0.8: automatic analysis + text adaptation preview ---
let autoTimer=null;
let lastAutoText='';
function getCalculationFunction(){
  if(typeof calculate==='function') return calculate;
  if(typeof runCalculation==='function') return runCalculation;
  if(typeof doCalculation==='function') return doCalculation;
  return null;
}
function scheduleAutoCalculation(){
  ...
  autoTimer=setTimeout(()=>{
    const fn=getCalculationFunction();
    if(fn){
      lastAutoText=text;
      fn();
    }
...
function updateAdapterPreview(){
  ...
  if(!input||!box)return;
  const rows=parse(input.value);
  if(!rows.length){
    box.style.display='none';
    return;
  }
  const body=rows.map((r,i)=>
    `<div class="adapter-row"><span>${i+1}. ${r.dims.join(' × ')} ${r.unit}</span><strong>${r.quantity} шт.</strong></div>`
  ).join('');
  box.innerHTML=`<div class="adapter-title">🧠 Адаптация текста</div>
    <div class="adapter-summary">Найдено: ${rows.length} поз. · ${rows.reduce((a,r)=>a+r.quantity,0)} мест</div>
    ${body}`;
...
function initAutoAnalysis(){
  ...
  const btn=$('adaptBtn');
  if(input){
    input.addEventListener('input',scheduleAutoCalculation);
    input.addEventListener('input',updateAdapterPreview);
  }
  if(btn)btn.addEventListener('click',updateAdapterPreview);
}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',initAutoAnalysis);
else initAutoAnalysis();
