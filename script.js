...
function updateConnectionStatus(){
  ...
  connectionStatus.textContent=online?'● Подключение активно':'● Офлайн-режим активен';
}
...
function unitNear(t,end){
  ...
  const m=s.match(/^\s*(мм|mm|см|cm|м|m)(?=\s|$|x|×)/);
  return m ? ({mm:'мм',cm:'см',m:'м'}[m[1]] || m[1]) : null;
}
function quantityFromContext(t,start,end){
  const before=t.slice(Math.max(0,start-80),start);
  const after=t.slice(end,Math.min(t.length,end+120));
  const qPatterns=[
  ...
  // Explicit quantity immediately before dimensions has highest priority.
  let mBefore = before.match(/(?:^|[^\d])(?:количеств(?:о|а)?\s*[:=]?\s*)?(\d+(?:[.,]\d+)?)\s*(?:шт\.?|штук|мест(?:а|о)?|pcs?)\s*$/i);
  if(mBefore)return Math.max(1,Math.round(num(mBefore[1])));
  // Forms after dimensions: "1200x800x600 мм × 7", "... × 7 шт."
  let m=after.match(/^\s*(?:мм|см|м|mm|cm|m)?\s*[x×]\s*(\d+(?:[.,]\d+)?)(?:\s*(?:шт\.?|штук|мест(?:а|о)?|pcs?))?/i);
  if(m)return Math.max(1,Math.round(num(m[1])));
  m=after.match(/^\s*(?:,|;|-)?\s*(?:количеств(?:о|а)?\s*[:=]?\s*)?(\d+(?:[.,]\d+)?)\s*(?:шт\.?|штук|мест(?:а|о)?|pcs?)\b/i);
  if(m)return Math.max(1,Math.round(num(m[1])));
  m=after.match(/(?:количеств(?:о|а)?\s*[:=]?\s*)(\d+(?:[.,]\d+)?)\s*(?:шт\.?|штук|мест(?:а|о)?|pcs?)\b/i);
  if(m)return Math.max(1,Math.round(num(m[1])));
  return 1;
}
...
function calculateNow(){
  state.items=parse(els.input.value);
  render();
}

els.calc.addEventListener('click',calculateNow);

// Automatic calculation: paste/input -> existing calculation engine.
// No duplicated parser and no dependency on a global "calculate" function.
let autoCalcTimer=null;
let lastCalculatedText='';
function scheduleAutoCalculate(){
  clearTimeout(autoCalcTimer);
  const text=els.input.value.trim();
  if(!text || text===lastCalculatedText)return;
  autoCalcTimer=setTimeout(()=>{
    lastCalculatedText=els.input.value.trim();
    calculateNow();
  },250);
}
els.input.addEventListener('input',scheduleAutoCalculate);
els.input.addEventListener('paste',()=>setTimeout(scheduleAutoCalculate,50));

// Adaptation is deliberately separate from automatic calculation.
// It extracts only recognized cargo data from arbitrary surrounding text.
function adaptText(){
  const source=els.input.value.trim();
  const rows=parse(source);
  const box=$('adapterPreview');
  if(!box)return;

  if(!rows.length){
    box.style.display='block';
    box.innerHTML='<div class="adapter-title">🧠 Адаптация текста</div><div class="adapter-empty">Не удалось надёжно найти группу габаритов.</div>';
    return;
  }

  const normalized=rows.map(x =>
    `${x.dims.join('x')} ${x.unit} × ${x.quantity}`
  ).join('\n');

  box.innerHTML=
    '<div class="adapter-title">🧠 Адаптированный запрос</div>'+
    '<textarea class="adapter-output" readonly></textarea>'+
    `<div class="adapter-summary">Найдено: ${rows.length} ${rows.length===1?'позиция':'позиции'} · ${rows.reduce((s,x)=>s+x.quantity,0)} мест</div>`+
    '<button type="button" class="adapt-apply-btn" id="applyAdaptedBtn">Применить и пересчитать</button>';
  box.querySelector('.adapter-output').value=normalized;
  box.style.display='block';

  $('applyAdaptedBtn').onclick=()=>{
    els.input.value=normalized;
    lastCalculatedText=normalized;
    calculateNow();
  };
}

const adaptBtn=$('adaptBtn');
if(adaptBtn)adaptBtn.addEventListener('click',adaptText);
